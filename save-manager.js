...
// Existing game code keeps using "skeletonMiningSave"; this layer transparently
...
(function(){
  ...
  function loadIndex(){
    ...
    return parsed&&typeof parsed==='object'
      ? {activeSlot:parsed.activeSlot??null,lastPlayedSlot:parsed.lastPlayedSlot??null,legacyMigrated:!!parsed.legacyMigrated}
      : {activeSlot:null,lastPlayedSlot:null,legacyMigrated:false};
  }
  ...
  // One-time migration. The original key is left untouched as a safety backup,
  // but legacyMigrated prevents a deleted slot from being resurrected later.
  const legacy=rawGet(LEGACY_KEY);
  if(!index.legacyMigrated&&!existingSlotNumbers().length&&legacy){
    ...
    index.legacyMigrated=true;
    saveIndex();
  }else if(legacy&&!index.legacyMigrated){
    // If slots already exist, an earlier version already migrated or created them.
    index.legacyMigrated=true;
    saveIndex();
  }
  ...
  function decorateSave(value){
    ...
    const now=Date.now();
    parsed._slotMeta={...(parsed._slotMeta||{}),createdAt:parsed._slotMeta?.createdAt||now,lastPlayed:now};
  ...
  window.SkeletonSaveManager={
    ...
    selectSlot(slot){
      ...
      index.activeSlot=slot;index.lastPlayedSlot=slot;writesEnabled=true;saveIndex();return true;
    },
    ...
    touchActive(){
      ...
      index.lastPlayedSlot=index.activeSlot;saveIndex();
    }
  };
})();
